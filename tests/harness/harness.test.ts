...
describe('test harness port isolation', () => {
  it('reserves unique OS-assigned ports in parallel and releases idempotently', async () => {
    const reservations = await Promise.all(
      Array.from({ length: 6 }, () => reserveLoopbackPort()),
    )
...
describe('test harness resource namespaces', () => {
  ...
  it('separates workers and emits provider-safe names', () => {
    const first = createTestNamespace({ runId: 'PR #123', suiteId: 'Long Storage Suite', workerId: 1 })
    const second = createTestNamespace({ runId: 'PR #123', suiteId: 'Long Storage Suite', workerId: 2 })
...
describe('test suite taxonomy', () => {
  ...
  it('loads the release taxonomy and assigns representative files once', async () => {
    ...
    expect(classifyTestFile(taxonomy, 'src/tools/store/store.test.ts').map(({ id }) => id)).toEqual(['unit'])
    expect(classifyTestFile(taxonomy, 'tests/integration/pb-api-record.test.ts').map(({ id }) => id)).toEqual(['component'])
    expect(classifyTestFile(taxonomy, 'tests/integration/storage.test.ts').map(({ id }) => id)).toEqual(['storage-contract'])
    expect(classifyTestFile(taxonomy, 'src/core/db-postgres-rls.test.ts').map(({ id }) => id)).toEqual(['postgres-rls-contract'])
  })
