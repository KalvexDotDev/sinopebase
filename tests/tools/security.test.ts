...
// crypto.ts
// ---------------------------------------------------------------------------
import { Equal, HS256, HS512, MD5, S256Challenge, SHA256, SHA512 } from '~/tools/security/crypto'
...
// encrypt.ts
// ---------------------------------------------------------------------------
import { Decrypt, Encrypt } from '~/tools/security/encrypt'
...
describe('encrypt.ts', () => {
  ...
  it('Encrypt rejects wrong key length', () => {
    ...
    expect(() =>
      Encrypt(new TextEncoder().encode('data'), '12345678901234567890123456789012345'),
    ).toThrow()
  })
