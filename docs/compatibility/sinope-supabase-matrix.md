...
- Storage currently buffers request bodies. Signed URLs use HMAC-SHA256 with per-bucket key derivation, NonceStore replay protection, and timing-safe comparison. They are single-use tokens — the v0.7 plan is to switch to stateless JWTs for Supabase-compatible reusable tokens.
