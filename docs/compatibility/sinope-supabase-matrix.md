...
- This is a Sinope workload contract, not an implementation of all GoTrue, PostgREST, Realtime, or Storage APIs. In particular, do not infer support for PKCE/MFA/email confirmation, RPC/OpenAPI/schema switching, composite/deep relationships, multi-replica Realtime, or Storage transformations/resumable uploads. OAuth (social + enterprise OIDC) and Realtime presence (track/untrack/diff/heartbeat) shipped in v0.6.2.
...
- Storage currently buffers request bodies. Signed URLs use HMAC-SHA256 with per-bucket key derivation, NonceStore replay protection, and timing-safe comparison. They are single-use tokens — switching to stateless JWTs for Supabase-compatible reusable tokens is planned.
