...
async function loadCronJobs(pool: Pool) {
  try {
    ...
    return rows.map((r) => ({
      ...
      schedule: r.schedule || '',
      handler: r.handler || '',
      lastRun: r.last_run?.toISOString?.() ?? null,
    }))
  } catch {
    return []
  }
...
export function createCronCrudPlugin(pool: Pool, isSuperuser: (r: Request) => boolean) {
  ...
  pool
    .query(
      "CREATE TABLE IF NOT EXISTS _crons (id TEXT PRIMARY KEY, label TEXT DEFAULT '', schedule TEXT DEFAULT '', handler TEXT DEFAULT '')",
    )
    .catch(() => {})
  ...
  app.post('/api/crons', async ({ request, body, set }) => {
    ...
    await pool
      .query('INSERT INTO _crons (id, label, schedule, handler) VALUES ($1,$2,$3,$4)', [
        id,
        label || '',
        schedule || '',
        handler || '',
      ])
      .catch(() => (set.status = 409))
  ...
  app.patch('/api/crons/:id', async ({ request, params, body, set }) => {
    ...
    if (sets.length > 0)
      await pool.query(`UPDATE _crons SET ${sets.join(', ')} WHERE id = $${i}`, [
        ...vals,
        params.id,
      ])
