...
import type { PostgrestResponse, PostgrestError } from './client'
...
class PostgrestClientImpl<T extends Record<string, unknown>> implements PostgrestClient<T> {
  ...
  select(columns = '*', options = {}): PostgrestFilterBuilder<T> {
    return new PostgrestFilterBuilderImpl<T>(
      this.baseUrl, this.apiKey, this.table,
      'GET', undefined, columns, options,
    )
  }
  ...
  insert(values: Partial<T> | Partial<T>[], options = {}): PostgrestFilterBuilder<T> {
    return new PostgrestFilterBuilderImpl<T>(
      this.baseUrl, this.apiKey, this.table,
      'POST', values, '*', options as Record<string, unknown>,
    )
  }
  ...
  async rpc<U = unknown>(fn: string, args?: Record<string, unknown>): Promise<PostgrestResponse<U>> {
    ...
    return { data: body as U, error: null, count: null, status: res.status, statusText: res.statusText }
  }
}
...
class PostgrestFilterBuilderImpl<T extends Record<string, unknown>>
  ...
  constructor(
    ...
    method: string, body: unknown, columns?: string, options: Record<string, unknown> = {},
  ) {
    this.baseUrl = baseUrl
    this.apiKey = apiKey
    this.table = table
  ...
  // Filters
  eq(column: string, value: unknown): this { this.filters.push(`${column}=eq.${encodeURIComponent(String(value))}`); return this }
  neq(column: string, value: unknown): this { this.filters.push(`${column}=neq.${encodeURIComponent(String(value))}`); return this }
  gt(column: string, value: unknown): this { this.filters.push(`${column}=gt.${encodeURIComponent(String(value))}`); return this }
  gte(column: string, value: unknown): this { this.filters.push(`${column}=gte.${encodeURIComponent(String(value))}`); return this }
  lt(column: string, value: unknown): this { this.filters.push(`${column}=lt.${encodeURIComponent(String(value))}`); return this }
  lte(column: string, value: unknown): this { this.filters.push(`${column}=lte.${encodeURIComponent(String(value))}`); return this }
  like(column: string, pattern: string): this { this.filters.push(`${column}=like.${encodeURIComponent(pattern)}`); return this }
  ilike(column: string, pattern: string): this { this.filters.push(`${column}=ilike.${encodeURIComponent(pattern)}`); return this }
  is(column: string, value: null | boolean): this { this.filters.push(`${column}=is.${value === null ? 'null' : String(value)}`); return this }
  in(column: string, values: unknown[]): this { this.filters.push(`${column}=in.(${values.map(v => encodeURIComponent(String(v))).join(',')})`); return this }
  contains(column: string, value: unknown): this { this.filters.push(`${column}=cs.${encodeURIComponent(JSON.stringify(value))}`); return this }
  or(filters: string): this { this.filters.push(`or=(${encodeURIComponent(filters)})`); return this }
  not(column: string, operator: FilterOperator, value: unknown): this { this.filters.push(`${column}=not.${operator}.${encodeURIComponent(String(value))}`); return this }
  ...
  limit(count: number): this { this.limitParam = count; return this }
  offset(count: number): this { this.offsetParam = count; return this }
  range(from: number, to: number): this { this.rangeParam = { from, to }; return this }
  single(): PromiseLike<{ data: T | null; error: PostgrestError | null }> { this.wantSingle = true; return this.executeSingle() }
  maybeSingle(): PromiseLike<{ data: T | null; error: PostgrestError | null }> { this.wantMaybeSingle = true; return this.executeSingle() }

  // Thenable — await the query directly
  then<TResult1 = PostgrestResponse<T[]>, TResult2 = never>(
    onfulfilled?: ((value: PostgrestResponse<T[]>) => TResult1 | PromiseLike<TResult1>) | null,
    onrejected?: ((reason: unknown) => TResult2 | PromiseLike<TResult2>) | null,
  ): PromiseLike<TResult1 | TResult2> {
