...
function mapVal(m: Record<string, unknown>, ...keys: string[]): unknown {
  ...
  if (!firstKey) throw new Error('invalid key path — empty key list')
  if (!(firstKey in m)) {
    throw new Error(`invalid key path - missing key "${firstKey}"`)
  }

  const result = m[firstKey]
...
function arrVal(m: unknown[], ...keys: string[]): unknown {
  const key0 = keys[0]
  if (key0 === undefined) throw new Error('invalid key path — empty key list')
  const idx = parseInt(key0, 10)
  if (Number.isNaN(idx) || idx < 0 || idx >= m.length) {
    throw new Error(`invalid key path - invalid or missing array index "${key0}"`)
  }
...
export class RecordFieldResolver implements FieldResolver {
   ...
   */
  resolve(_fieldName: string): ResolverResult | Error {
  ...
  registerRuleJoin(collectionName: string, tableAlias: string): void {
    const existingIdx = this.listRuleJoins.findIndex((j) => j.tableAlias === tableAlias)
    if (existingIdx >= 0) {
  ...
  resolveStaticRequestField(...path: string[]): ResolverResult | Error {
    ...
    let lastProp = path[lastIdx]
    if (!lastProp) throw new Error('invalid path — empty segment array')
    let modifier = ''
    try {
