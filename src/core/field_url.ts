/**
 * URLField — stores a single URL string with domain validation.
 *
 * Port of PocketBase's core/field_url.go (Go -> TypeScript).
...
/**
 * URLField defines a "url" type field for storing a single URL string value.
 *
...
export class URLField implements Field {
  ...
  readonly type: string = FieldTypeURL
  ...
  /** Validates a field value against URL format and domain restrictions. */
  validateValue(value: string): string | null {
    ...
    try {
      ...
      if (this.onlyDomains.length > 0 && !this.onlyDomains.includes(url.host)) {
        return 'URL domain is not allowed'
      }

      if (this.exceptDomains.length > 0 && this.exceptDomains.includes(url.host)) {
        return 'URL domain is not allowed'
      }
...
RegisterField(FieldTypeURL, () => new URLField())
