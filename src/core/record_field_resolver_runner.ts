...
import { jsonArrayLength, jsonEach, jsonExtract } from '~/tools/dbutils/json'
import { Columnify } from '~/tools/inflector/inflector'
import { ToUniqueStringSlice } from '~/tools/list/list'
...
import type { CollectionStub, FieldStub, MultiValuer } from './record_field_resolver'
import {
  ChangedModifier,
  EachModifier,
  FieldNameEmail,
...
class Runner {
  ...
  run(): ResolverResult | Error {
    ...
    if (this.resolver.allowedFields().length > 0 && !this.fieldMatchesAllowed(this.fieldName)) {
    ...
    if (this.activeProps[0] === '@request') {
      ...
      if (this.fieldName.startsWith('@request.body.') && this.activeProps.length > 2) {
        let name: string
        let modifier: string
        const prop2 = this.activeProps[2]
        if (!prop2) throw new Error('activeProps[2] is required for resolve action')
        try {
          const r = splitModifier(prop2)
          name = r[0]
          modifier = r[1]
        } catch {
          name = prop2
        ...
        if (bodyField) {
          ...
          if (bodyField.type === 'relation' && this.activeProps.length > 3) {
  ...
  protected processCollectionField(): ResolverResult | Error {
    ...
    try {
      const collName = collectionParts[0]
      if (!collName) throw new Error('collection name is required')
      collection = this.resolver.loadCollection(collName)
    } catch (_err) {
      return new Error(
    ...
    if (collectionParts.length === 2 && collectionParts[1] !== '') {
      this.activeTableAlias =
        Columnify(`__collection_alias_${collectionParts[1]}`) + this.resolver.joinAliasSuffix
    } else {
      this.activeTableAlias =
    ...
    this.resolver.registerJoin(Columnify(collection.name), this.activeTableAlias, '')
   ...
   */
  protected processRequestBodyRelationField(bodyField: FieldStub): ResolverResult | Error {
    const ri = this.resolver.requestInfo

    // We need the raw relation field from the collection schema
    const relFieldConfig = this.resolver.baseCollection.fields.getByName(bodyField.name)
    if (!relFieldConfig) {
      return new Error(`failed to initialize data relation field "${bodyField.getName()}"`)
    }

    let relCollection: CollectionStub
    try {
      // Look up the related collection id from the field config
      const collId = (relFieldConfig as unknown as Record<string, unknown>).collectionId || ''
      relCollection = this.resolver.loadCollection(collId)
    } catch (_err) {
      return new Error(`failed to load collection from data field "${bodyField.getName()}"`)
    }
    ...
    const isMultiple = (relFieldConfig as unknown as Record<string, unknown>).isMultiple as boolean
    if (isMultiple) {
      this.withMultiMatch = true
    }
   ...
   */
  protected processRequestBodyChangedModifier(bodyField: FieldStub): ResolverResult | Error {
   ...
   */
  protected processRequestBodyLowerModifier(bodyField: FieldStub): ResolverResult | Error {
    const ri = this.resolver.requestInfo
    const rawValue = ri?.body?.[bodyField.getName()] ?? ''
    const strValue = String(rawValue)

    const placeholder = `infoLower${bodyField.getName()}${Math.random().toString(36).slice(2, 10)}`
   ...
   */
  protected processRequestBodyLengthModifier(bodyField: FieldStub): ResolverResult | Error {
    const ri = this.resolver.requestInfo
   ...
   */
  protected processRequestBodyEachModifier(bodyField: FieldStub): ResolverResult | Error {
    const ri = this.resolver.requestInfo
    const bodyItems = toSlice(ri?.body?.[bodyField.getName()])

    const bodyItemsRaw = JSON.stringify(bodyItems)
    const placeholder = `dataEach${Math.random().toString(36).slice(2, 12)}`
    ...
    const isMultivaluer = (bodyField as unknown as MultiValuer).isMultiple?.() ?? false
  ...
  protected processActiveProps(): ResolverResult | Error {
    ...
    for (let i = 0; i < totalProps; i++) {
      ...
      if (prop === undefined) throw new Error(`activeProps[${i}] is required`)
      ...
      // Hidden field check
      if (field?.getHidden?.() && !this.resolver.getAllowHiddenFields()) {
      ...
      if (i >= maxNestedRels) {
        return new Error(`max nested relations reached for field "${prop}"`)
      }
      ...
      if (!field) {
        ...
        if (!backCollectionName) throw new Error('back-relation collection name is required')
        const backFieldName = parts[2]
        if (!backFieldName) throw new Error('back-relation field name is required')
        ...
        try {
          ...
          return new Error(`failed to load back relation field "${prop}" collection`)
        }
        ...
        if (!backField) {
          ...
          return new Error(`missing back relation field "${backFieldName}"`)
        }
        ...
        if (backField.type !== 'relation') {
          ...
          return new Error(`invalid back relation field "${backFieldName}"`)
        }

        if (backField.getHidden?.() && !this.resolver.getAllowHiddenFields()) {
          return new Error(`non-filterable back relation field "${backField.getName()}"`)
        }
        ...
        const isBackRelMultiple = (backField as unknown as MultiValuer).isMultiple?.() ?? false
        ...
        if (!isBackRelMultiple) {
          ...
          const jsonEachExpr = jsonEach(`${newTableAlias}.${cleanBackFieldName}`)
        ...
        const newTableAlias2 = `${this.multiMatchActiveTableAlias}_${cleanProp}`
        ...
        if (!isBackRelMultiple) {
          ...
          const jsonEachExpr2 = jsonEach(`${newTableAlias2}.${cleanBackFieldName}`)
          this.multiMatch.joins.push({
      ...
      const relCollectionName = (field as unknown as Record<string, unknown>).collectionId as string
      let relCollection: CollectionStub
      try {
      ...
      const isRelMultiple = (field as unknown as MultiValuer).isMultiple?.() ?? false
  ...
  protected finalizeActivePropsProcessing(
    ...
    const cleanFieldName = Columnify(field.getName())
    const isMultivaluer = !!(field as unknown as MultiValuer).isMultiple?.()
    ...
    if (modifier === LengthModifier && isMultivaluer) {
      ...
      if (this.withMultiMatch) {
        const jePair2 = `${this.multiMatchActiveTableAlias}.${cleanFieldName}`
    ...
    if (modifier === EachModifier && isMultivaluer) {
      ...
      if ((field as unknown as MultiValuer).isMultiple?.()) {
        this.withMultiMatch = true
      }

      if (this.withMultiMatch) {
        const jePair2 = `${this.multiMatchActiveTableAlias}.${cleanFieldName}`
        const jeAlias2 = `__je_${this.multiMatchActiveTableAlias}_${cleanFieldName}`
   ...
   */
  protected resolveParamPlaceholder(_value: string): string {
    const placeholder = `p${Math.random().toString(36).slice(2, 12)}`
    return `{:${placeholder}}`
  }
}
