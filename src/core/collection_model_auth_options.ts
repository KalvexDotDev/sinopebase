...
export class PasswordAuthConfig {
  ...
  validate(): string[] {
    ...
    if (this.enabled) {
      ...
      if (unique.length === 0) {
        errors.push(
          'identityFields: at least one identity field is required when password auth is enabled',
        )
      }
    }
...
export class CollectionAuthOptions {
  ...
  static fromJSON(data: Record<string, unknown>): CollectionAuthOptions {
    ...
    if (typeof data.verificationRequired === 'boolean')
      opts.verificationRequired = data.verificationRequired
    ...
    if (data.passwordResetToken && typeof data.passwordResetToken === 'object') {
      opts.passwordResetToken = TokenConfig.fromJSON(
        data.passwordResetToken as Record<string, unknown>,
      )
    }
    ...
    if (data.verificationToken && typeof data.verificationToken === 'object') {
      opts.verificationToken = TokenConfig.fromJSON(
        data.verificationToken as Record<string, unknown>,
      )
    }
    ...
    if (data.verificationTemplate && typeof data.verificationTemplate === 'object') {
      opts.verificationTemplate = EmailTemplate.fromJSON(
        data.verificationTemplate as Record<string, unknown>,
      )
    }
    if (data.resetPasswordTemplate && typeof data.resetPasswordTemplate === 'object') {
      opts.resetPasswordTemplate = EmailTemplate.fromJSON(
        data.resetPasswordTemplate as Record<string, unknown>,
      )
    }
    if (data.confirmEmailChangeTemplate && typeof data.confirmEmailChangeTemplate === 'object') {
      opts.confirmEmailChangeTemplate = EmailTemplate.fromJSON(
        data.confirmEmailChangeTemplate as Record<string, unknown>,
      )
    }
