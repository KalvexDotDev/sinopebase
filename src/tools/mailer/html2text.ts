...
function* tokenize(html: string): Generator<Token> {
  ...
  while (i < html.length) {
    ...
    if (html[i] === '<') {
      ...
      const cleanContent = isSelfClose ? tagContent.slice(0, -1).trim() : tagContent
...
function parseAttributes(attrsStr: string): Record<string, string> {
  ...
  let match = attrRegex.exec(attrsStr)
  while (match !== null) {
  ...
  for (const token of tokenize(htmlDocument)) {
    if (token.type === 'opentag' || token.type === 'selfclose') {
      ...
      if (tag === 'br') {
        output.push('\r\n')
        canAddNewLine = false
      } else if (canAddNewLine && !INLINE_TAGS.has(tag) && token.type === 'opentag') {
        output.push('\r\n')
        canAddNewLine = false
      }
    }
    ...
    if (token.type === 'closetag') {
      ...
      if (canAddNewLine && !INLINE_TAGS.has(token.tag)) {
        output.push('\r\n')
        canAddNewLine = false
      }
    }
