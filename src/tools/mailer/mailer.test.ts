...
import { addressToStrings, Message } from './mailer.ts'
...
describe('addressToStrings', () => {
  it('returns bare email when withName is false', () => {
    const result = addressToStrings([{ name: 'Alice', address: 'alice@example.com' }], false)
    expect(result).toEqual(['alice@example.com'])
  })

  it('returns formatted name+email when withName is true', () => {
    const result = addressToStrings([{ name: 'Alice', address: 'alice@example.com' }], true)
