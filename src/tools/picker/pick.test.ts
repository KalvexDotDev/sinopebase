...
describe("Pick", () => {
  ...
  it("excludes nested fields with wildcard + exclusion", () => {
    ...
    expect((result as any).b).toEqual({ b2: 22 });
  });
  ...
  it("deep clones data through JSON", () => {
    ...
    (result as any).a.nested = "modified";
    expect((data.a as any).nested).toBe("original");
  });

  it("handles deeply nested paths", () => {
    const data = { a: { b: { c: { d: 42 } } } };
    const result = Pick(data, "a.b.c.d") as Record<string, unknown>;
    expect((result as any).a.b.c.d).toBe(42);
  });
  ...
  it("excludes deeply nested sub-fields with wildcard", () => {
    ...
    expect((result as any).a.x).toEqual({ p: 1 });
    expect((result as any).a.y).toBe(3);
    expect((result as any).b).toBe(4);
  });
  ...
  it("wildcard with multiple nested exclusions", () => {
    ...
    const result = Pick(
      data,
      "*,-user.ssn,-config.secret",
    ) as Record<string, unknown>;
    expect((result as any).visible).toBe("ok");
    expect((result as any).user).toEqual({
      name: "Alice",
      email: "a@x.com",
    });
    expect((result as any).config).toEqual({ public: "hello" });
  });
