...
describe("fireAndForget", () => {
  ...
  it("catches async rejections without crashing", async () => {
    ...
    console.error = () => { errorCaught = true; };
    try {
      fireAndForget(async () => { throw new Error("async error"); });
      await Promise.resolve();
      await Promise.resolve();
      expect(errorCaught).toBe(true);
    } finally {
...
describe("safeWrap", () => {
  ...
  it("returns Error when function throws", async () => {
    const wrapped = safeWrap(() => { throw new Error("boom"); });
    const result = await wrapped();
  ...
  it("returns Error when async function rejects", async () => {
    const wrapped = safeWrap(async () => { throw new Error("async boom"); });
    const result = await wrapped();
    expect(result).toBeInstanceOf(Error);
  ...
  it("converts non-Error throws to Error", async () => {
    const wrapped = safeWrap(() => { throw "string error"; });
    const result = await wrapped();
    expect(result).toBeInstanceOf(Error);
  });
});
