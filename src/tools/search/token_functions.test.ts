...
import { concatUniqueParams, tokenFunctions } from './token_functions'
...
describe('tokenFunctions', () => {
  ...
  describe('strftime', () => {
    it('returns Error with no args', () => {
      const result = tokenFunctions.strftime?.(() => ({ identifier: '', params: {} }))
      expect(result instanceof Error).toBe(true)
    })
    ...
    it('builds to_char expression with format only', () => {
      const result = tokenFunctions.strftime?.(() => ({ identifier: "'%Y'", params: {} }), {
        type: 'text',
        literal: '%Y',
      })
      expect(result instanceof Error).toBe(false)
