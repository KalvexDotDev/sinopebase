...
import { buildFilterExpr, MAX_FILTER_LENGTH } from './filter'
import type { FieldResolver } from './simple_field_resolver'
import type { SortField } from './sort'
import { buildSortExpr, parseSort } from './sort'
  ...
  buildOrderBy(firstFromTable?: string): OrderByClause[] {
    ...
    for (const sortField of this.sort) {
      ...
      try {
        ...
        throw new Error(`invalid sort field "${sortField.name}": ${(err as Error).message}`)
      }

      // Ensure _rowid_ (ctid) expressions are prefixed with the first FROM table
      if (sortField.name === '@rowid' && !expr.includes('.') && firstFromTable) {
