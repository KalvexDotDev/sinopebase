...
describe('CreateZipArchive', () => {
  ...
  it('creates a zip archive from a directory with a custom archive name', async () => {
    ...
    const entries = zip.getEntries()
    expect(entries.length).toBe(1)
    expect(entries[0]?.entryName).toBe('archive_root/file.txt')
  })
  ...
  it('skips a directory listed in skipPaths', async () => {
    ...
    const zipPath = join(destDir, 'output.zip')
    await CreateZipArchive([{ path: testDir, name: 'root' }], zipPath, ['skip_me'])
  ...
  it('skips a file listed in skipPaths', async () => {
    ...
    const zipPath = join(destDir, 'output.zip')
    await CreateZipArchive([{ path: testDir, name: 'root' }], zipPath, ['skip.txt'])
  ...
  it('throws if a source path does not exist', async () => {
    ...
    expect(CreateZipArchive([{ path: missingPath }], join(destDir, 'out.zip'))).rejects.toThrow()
  })
  ...
  it('creates an archive from multiple sources', async () => {
    ...
    try {
      ...
      await CreateZipArchive(
        [{ path: join(testDir, 'a.txt') }, { path: dir2, name: 'source2' }],
        zipPath,
      )
      ...
      expect(names).toEqual(['a.txt', 'source2/b.txt'])
    } finally {
