...
describe('ExtractZipArchive', () => {
  ...
  it('extracts nested directory structure', async () => {
    ...
    expect(existsSync(join(extractSubdir, 'root', 'a', 'b', 'nested.txt'))).toBe(true)
    expect(readFileSync(join(extractSubdir, 'root', 'root.txt'), 'utf-8')).toBe('root content')
    expect(readFileSync(join(extractSubdir, 'root', 'a', 'b', 'nested.txt'), 'utf-8')).toBe(
      'nested content',
    )
  })
  ...
  it('safely handles path traversal entries (adm-zip normalizes them)', async () => {
    ...
    maliciousZip.addFile('../../../etc/passwd', Buffer.from('content'))
  ...
  it('throws on invalid zip data', async () => {
    ...
    expect(ExtractZipArchive(invalidData, extractDir)).rejects.toThrow()
  })

  it('throws when zip file does not exist', async () => {
    const missingPath = join(testDir, 'nonexistent.zip')

    expect(ExtractZipArchive(missingPath, extractDir)).rejects.toThrow()
  })
})
