...
describe("Broker", () => {
  ...
  it("broadcastTo sends to specific client ids", () => {
    ...
    for (let i = 0; i < 5; i++) {
      const client = new DefaultClient(`c${i}`);
      client.onMessage = () => { received.push(`c${i}`); };
      broker.register(client);
    }
